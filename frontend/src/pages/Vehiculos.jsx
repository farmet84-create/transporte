...
// Hook para detectar si es móvil con JS (no depende de Tailwind)
function useEsMobil() {
  const [esMobil, setEsMobil] = useState(() => window.innerWidth < 900)
  useEffect(() => {
    const fn = () => setEsMobil(window.innerWidth < 900)
...
const BadgeVencimiento = ({ fecha }) => {
  ...
  if (dias < 0) return <div style={{ display:'flex', alignItems:'center', gap:4 }}><AlertTriangle style={{ width:12, height:12, color:'#dc2626' }} /><span style={{ fontSize:12, color:'#dc2626', fontWeight:500 }}>Vencido</span></div>
  if (dias <= 30) return <div style={{ display:'flex', alignItems:'center', gap:4 }}><AlertTriangle style={{ width:12, height:12, color:'#d97706' }} /><span style={{ fontSize:12, color:'#d97706', fontWeight:500 }}>Vence en {dias}d</span></div>
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      <div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(140px, 1fr))', gap:12 }}>
          <Field label="Placa *"><input value={form.placa} onChange={e=>set('placa',e.target.value.toUpperCase())} placeholder="ABC123" className="input font-mono uppercase" maxLength={10} /></Field>
          <Field label="Tipo *"><select value={form.tipo} onChange={e=>set('tipo',e.target.value)} className="input">{TIPOS.map(t=><option key={t} value={t}>{t.replace('_',' ')}</option>)}</select></Field>
          <Field label="Marca *"><input value={form.marca} onChange={e=>set('marca',e.target.value)} placeholder="Kenworth, Hino..." className="input" /></Field>
          <Field label="Modelo *"><input value={form.modelo} onChange={e=>set('modelo',e.target.value)} placeholder="T800, FC..." className="input" /></Field>
          <Field label="Año *"><input type="number" value={form.anio} onChange={e=>set('anio',e.target.value)} min="1990" max="2030" className="input" /></Field>
          <Field label="Combustible"><select value={form.tipo_combustible} onChange={e=>set('tipo_combustible',e.target.value)} className="input">{COMBUSTIBLES.map(c=><option key={c} value={c}>{c}</option>)}</select></Field>
          <Field label="Capacidad (kg)"><input type="number" value={form.capacidad_carga_kg} onChange={e=>set('capacidad_carga_kg',e.target.value)} placeholder="35000" className="input" /></Field>
          <Field label="Rendimiento (km/gal)"><input type="number" step="0.1" value={form.rendimiento_km_galon} onChange={e=>set('rendimiento_km_galon',e.target.value)} placeholder="8.5" className="input" /></Field>
          <Field label="Color"><input value={form.color} onChange={e=>set('color',e.target.value)} className="input" /></Field>
          <Field label="Propietario"><input value={form.propietario} onChange={e=>set('propietario',e.target.value)} placeholder="Si es de tercero" className="input" /></Field>
          <Field label="N° Motor"><input value={form.numero_motor} onChange={e=>set('numero_motor',e.target.value)} className="input" /></Field>
          <Field label="N° Chasis"><input value={form.numero_chasis} onChange={e=>set('numero_chasis',e.target.value)} className="input" /></Field>
        </div>
      </div>
      <div>
        <p style={{ fontSize:13, fontWeight:700, color:'#374151', borderBottom:'1px solid #f1f5f9', paddingBottom:6, marginBottom:12 }}>🛡️ SOAT</p>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(140px, 1fr))', gap:12 }}>
          <Field label="Vencimiento SOAT"><input type="date" value={form.soat_vencimiento||''} onChange={e=>set('soat_vencimiento',e.target.value)} className="input" /></Field>
          <Field label="Aseguradora"><input value={form.soat_aseguradora||''} onChange={e=>set('soat_aseguradora',e.target.value)} placeholder="Sura, Bolívar..." className="input" /></Field>
          <Field label="N° Póliza"><input value={form.soat_numero_poliza||''} onChange={e=>set('soat_numero_poliza',e.target.value)} className="input" /></Field>
        </div>
      </div>
      <div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(140px, 1fr))', gap:12 }}>
          <Field label="Vencimiento"><input type="date" value={form.tecnomecanica_vencimiento||''} onChange={e=>set('tecnomecanica_vencimiento',e.target.value)} className="input" /></Field>
          <Field label="N° Certificado"><input value={form.tecnomecanica_numero||''} onChange={e=>set('tecnomecanica_numero',e.target.value)} className="input" /></Field>
        </div>
      </div>
      <div>
      ...
      <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
        ...
        <button onClick={() => onGuardar(form)} disabled={cargando} className="btn-primary" style={{ display:'flex', alignItems:'center', gap:6 }}>
          <Save style={{ width:14, height:14 }} />{cargando ? 'Guardando...' : 'Guardar'}
        </button>
      </div>
    </div>
  )
...
export default function Vehiculos() {
  ...
  const guardarNuevo = async (form) => {
    ...
    setGuardando(true)
    try { await vehiculosAPI.crear(form); toast.success('Vehículo creado'); setModal(null); cargar() }
  ...
  const guardarEdicion = async (form) => {
    setGuardando(true)
    try { await vehiculosAPI.actualizar(modal.id, form); toast.success('Actualizado'); setModal(null); cargar() }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, marginTop:4, flexWrap:'wrap' }}>
            ...
            {alertas > 0 && (
              <span style={{ display:'flex', alignItems:'center', gap:4, fontSize:12, color:'#d97706', fontWeight:600, background:'#fffbeb', padding:'2px 10px', borderRadius:20, border:'1px solid #fde68a' }}>
                <AlertTriangle style={{ width:12, height:12 }} />{alertas} documentos por vencer
              </span>
            )}
          </div>
        </div>
        <button onClick={() => setModal('nuevo')} className="btn-primary" style={{ display:'flex', alignItems:'center', gap:6, fontSize:13 }}>
          <Plus style={{ width:14, height:14 }} /> Nuevo vehículo
        </button>
      </div>

      {/* Búsqueda */}
      <div style={{ background:'#fff', borderRadius:12, padding:14, border:'1px solid #f1f5f9', boxShadow:'0 1px 4px rgba(0,0,0,0.05)', display:'flex', gap:10 }}>
        <div style={{ flex:1, position:'relative' }}>
          <Truck style={{ position:'absolute', left:10, top:'50%', transform:'translateY(-50%)', width:15, height:15, color:'#9ca3af' }} />
          <input value={busqueda} onChange={e => { setBusqueda(e.target.value.toUpperCase()); setPagina(1) }}
            placeholder="Buscar por placa..." className="input font-mono uppercase" style={{ paddingLeft:34, fontSize:13 }} />
        </div>
        ...
        {busqueda && (
          <button onClick={() => { setBusqueda(''); setPagina(1) }} className="btn-secondary">
        ...
        esMobil ? (
          ...
          <div>
            {vehiculos.length === 0 ? (
              ...
              const alerta = (diasParaVencer(v.soat_vencimiento) !== null && diasParaVencer(v.soat_vencimiento) <= 30) ||
                             (diasParaVencer(v.tecnomecanica_vencimiento) !== null && diasParaVencer(v.tecnomecanica_vencimiento) <= 30)
              return (
                <div key={v.id} style={{ background:'#fff', borderRadius:14, border:`1px solid ${alerta?'#fde68a':'#f1f5f9'}`, padding:16, marginBottom:10, boxShadow:'0 1px 4px rgba(0,0,0,0.05)' }}>
                  ...
                  <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
                    <button onClick={() => setModal(v)} style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 14px', background:'#eff6ff', border:'none', borderRadius:8, cursor:'pointer', color:'#4f46e5', fontSize:13, fontWeight:500 }}>
                      <Edit2 style={{ width:13, height:13 }} /> Editar
                    </button>
                    <button onClick={() => eliminar(v)} style={{ padding:'7px 10px', background:'#fef2f2', border:'none', borderRadius:8, cursor:'pointer', color:'#dc2626' }}>
                ...
                <tbody>
                  {vehiculos.length === 0 ? (
                    ...
                    <tr key={v.id} style={{ borderBottom:'1px solid #f3f4f6' }}
                      ...
                      <td style={{ padding:'10px 16px' }}><span style={{ fontFamily:'monospace', fontWeight:700, background:'#f3f4f6', padding:'3px 8px', borderRadius:6, fontSize:12, color:'#111827' }}>{v.placa}</span></td>
                      ...
                      <td style={{ padding:'10px 16px' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:6, justifyContent:'flex-end' }}>
                          <button onClick={() => setModal(v)} style={{ padding:'5px', background:'transparent', border:'none', borderRadius:6, cursor:'pointer', color:'#4f46e5' }}><Edit2 style={{ width:15, height:15 }} /></button>
                          <button onClick={() => eliminar(v)} style={{ padding:'5px', background:'transparent', border:'none', borderRadius:6, cursor:'pointer', color:'#ef4444' }}><Trash2 style={{ width:15, height:15 }} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {modal === 'nuevo' && <Modal titulo="Nuevo vehículo" onClose={() => setModal(null)}><FormVehiculo onGuardar={guardarNuevo} onCancelar={() => setModal(null)} cargando={guardando} /></Modal>}
      {modal && modal !== 'nuevo' && <Modal titulo={`Editar — ${modal.placa}`} onClose={() => setModal(null)}><FormVehiculo inicial={modal} onGuardar={guardarEdicion} onCancelar={() => setModal(null)} cargando={guardando} /></Modal>}
    </div>
  )
}
