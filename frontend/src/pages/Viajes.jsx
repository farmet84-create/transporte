...
export default function Viajes() {
  ...
  const eliminar = async (v, e) => {
    ...
    if (!confirm(`¿Eliminar el viaje ${v.numero_viaje}?`)) return
    try { await viajesAPI.eliminar(v.id); toast.success('Viaje eliminado'); cargar() }
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          <Link to="/viajes/nuevo" className="btn-primary" style={{ display:'flex', alignItems:'center', gap:6, fontSize:13 }}>
            <Plus style={{ width:14, height:14 }} /> {esMobil ? 'Nuevo' : 'Nuevo viaje'}
          </Link>
        </div>
      </div>
      ...
      {mostrarFiltros && (
        <div style={{ background:'#fff', borderRadius:12, padding:16, border:'1px solid #e5e7eb', boxShadow:'0 1px 4px rgba(0,0,0,0.05)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
            ...
            {hayFiltros && <button onClick={limpiar} style={{ fontSize:12, color:'#dc2626', background:'transparent', border:'none', cursor:'pointer', display:'flex', alignItems:'center', gap:4 }}><X style={{ width:12, height:12 }} /> Limpiar</button>}
          </div>
          ...
          <button onClick={() => { setPagina(1); cargar() }} className="btn-primary" style={{ marginTop:12, display:'flex', alignItems:'center', gap:6, fontSize:13 }}>
        ...
        esMobil ? (
          ...
          <div>
            {viajes.length === 0 ? (
              ...
              <div key={v.id} style={{ background:'#fff', borderRadius:14, border:'1px solid #f1f5f9', padding:16, marginBottom:10, boxShadow:'0 1px 4px rgba(0,0,0,0.05)' }}>
                ...
                <p style={{ fontSize:12, color:'#6b7280', margin:'0 0 10px' }}>🧑 {v.conductor} · 🏢 {v.cliente}</p>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, padding:'10px 0', borderTop:'1px solid #f3f4f6' }}>
                  <div style={{ textAlign:'center' }}>
                    ...
                    <p style={{ fontSize:12, fontWeight:700, color:'#1d4ed8', margin:'2px 0 0' }}>{formatCOP(v.valor_flete_cobrado)}</p>
                  </div>
                  <div style={{ textAlign:'center' }}>
                ...
                <div style={{ display:'flex', gap:8, marginTop:10, justifyContent:'flex-end' }}>
                  ...
                  <button onClick={(e) => eliminar(v, e)} style={{ padding:'6px 10px', background:'#fef2f2', border:'none', borderRadius:8, cursor:'pointer', color:'#dc2626' }}>
                    ...
                    <tr key={v.id} style={{ borderBottom:'1px solid #f3f4f6' }}
                      ...
                      <td style={{ padding:'10px 16px' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:6, justifyContent:'flex-end' }}>
                          ...
                          <button onClick={(e) => eliminar(v,e)} style={{ padding:'4px', background:'transparent', border:'none', borderRadius:6, cursor:'pointer', color:'#ef4444' }}>
